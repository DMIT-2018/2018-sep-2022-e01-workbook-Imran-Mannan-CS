...
void PlaylistTrack_AddTrack(string playlistname, string username, int trackid)
{
	...
	playlisttrackexists.TrackId = trackid;
		...
		this means that the IDENTITY value for the new playList Does NOT yet exists. The value on the playlist instance (playlistexists)
		is zero.
		thus we have a serious problem
		
		Solution
		it is built into EntityFramework software and is base on using the navigational proprty is playlists pointing to its "child"
		...
		INSTEAD. do the staging using the syntax of "parent.navigational property.Add(xxxxxx)"
